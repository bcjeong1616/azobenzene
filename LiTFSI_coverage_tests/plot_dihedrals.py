...
def dihedral_pbc(x0,x1,x2,x3,Box):
    ...
    try:
        ...
        print(fr"dihedral failed to compute for vectors: {a} and {b}")
        dihedral = 0
    return dihedral
...
for input_gsd in input_arr:
    ...
    for frame in traj[-50:-1]:
        ...
        for group in dihedralGroups:
            dihedral = dihedral_pbc(pos[group[0]],pos[group[1]],pos[group[2]],pos[group[3]],box)
            dihedral = dihedral*(180/math.pi)
            dihedralsList = np.append(dihedralsList,dihedral)
        print(len(dihedralsList))
    
    #Plot the list of dihedrals as a histogram
    plt.hist(dihedralsList,bins=200,range=(0,180),color=plt.cm.Reds(0.3))
    plt.xlabel(rf'TFSI dihedral angle [deg]')
