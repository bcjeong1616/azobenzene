...
def equilibrated(job):
    return job.doc['equi_step'] >= 1e6 # 1000 ns

# @MyProject.label
# def dilatometry(job):
#     return job.isfile("dilatometry.gsd")
...
@MyProject.operation
def initialize(job):
    simulator = Simulator(job)
    simulator.initialize_system()
    print("initialized: ",job.id)

# if simulation not yet equilibrated - equilibrate!
@MyProject.post(equilibrated)
@MyProject.pre(initialized)
@MyProject.operation
def equilibrate(job):
    simulator = Simulator(job)
    simulator.equilibrate(init_path='./workspace/'+job.id+'/',kT=job.sp["kT"],P=job.sp["P"])
    # os.system(f"python3 ./scripts/equi.hoomd {job.id}")
    print("equilibrated: ",job.id,' ', job.doc['equi_step']," steps done")
